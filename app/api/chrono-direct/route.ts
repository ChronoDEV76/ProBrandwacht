// app/api/chrono-direct/route.ts
...
export async function POST(req: Request) {
  try {
    ...
    const { error: insErr } = await supabase.from("direct_requests").insert({
      ...
      source: body.source ?? "chrono-direct",
    });
    ...
    const htmlOps = `
      ...
      <hr/>
      <p style="color:#64748b">Bron: ${escapeHtml(body.source || "chrono-direct")} • AVG: ${
    ...
    const htmlUser = `
      ...
      Indien het binnen <strong>ProSafetyMatch</strong> past, regelen we de inzet transparant en DBA-proof. 
